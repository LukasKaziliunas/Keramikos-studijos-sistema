...
exports.save = function(material, amount, potteryId){
    ...
    var sql = "INSERT INTO `potterymaterial` (`fk_Material`, `fk_Pottery`, `amount`) VALUES ( ? , ? , ? )";
    var inserts = [material, potteryId, amount];
    sql = format(sql, inserts);
    return mysql.insert(sql);
}

exports.getByPottery2 = function(potteryId){
    var sql = "SELECT * FROM `potterymaterial` WHERE fk_Pottery = ?";
    sql = format(sql, potteryId);
    return mysql.query(sql);
}

exports.getByPottery = function(potteryId){
    var sql = "SELECT potterymaterial.id as id, potterymaterial.amount as potteryMaterialAmount,\
